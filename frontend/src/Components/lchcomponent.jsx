...
const GraphComponent = ({
  ...
  const updateSummary = (points) => {
    if (points) {
      const totalPoint = points.find((p) => p.series.name === 'Total');
      const targetPoint = points.find((p) => p.series.name === 'Target');

      if (totalPoint && targetPoint) {
        const totalValue = totalPoint.y;
        const targetValue = targetPoint.y;
  ...
  useEffect(() => {
    if (chartRef.current) {
      chartRef.current.chart.update({
        ...
        tooltip: {
          formatter: function () {
            updateSummary(this.points);
            return latestSummaryRef.current;
          }
        }
      });
    }
